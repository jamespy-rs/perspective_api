use anyhow::Result;
use perspective_api::client::{AnalyzeCommentError, AnalyzeCommentErrorResponse, AnalyzeCommentRequest, PerspectiveClient};
use std::error::Error;

// These tests highlight if Perspective change their API by calling it directly and comparing to the last known response.
const PERSPECTIVE_API_KEY: &str = include_str!("../resources/key");

#[tokio::test]
async fn test_text_empty() -> Result<(), Box<dyn Error>> {
    let body = include_str!("../src/client/v1alpha1/models/examples/text_empty_request.json");
    let request: AnalyzeCommentRequest = serde_json::from_str(body).unwrap();
    let body = include_str!("../src/client/v1alpha1/models/examples/text_empty_error.json");
    let error: AnalyzeCommentError = serde_json::from_str::<AnalyzeCommentErrorResponse>(body).unwrap().into();

    assert_eq!(error, PerspectiveClient::new(PERSPECTIVE_API_KEY).analyze(request).await.unwrap_err());
    Ok(())
}
