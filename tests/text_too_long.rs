...
#[tokio::test]
async fn test_text_too_long() -> Result<(), Box<dyn Error>> {
    let body = include_str!("../src/client/v1alpha1/models/examples/text_too_long_request.json");
    let request: AnalyzeCommentRequest = serde_json::from_str(body)?;
    let body = include_str!("../src/client/v1alpha1/models/examples/text_too_long_error.json");
    let error: AnalyzeCommentError = serde_json::from_str::<AnalyzeCommentErrorResponse>(body).unwrap().into();

    assert_eq!(error, PerspectiveClient::new(PERSPECTIVE_API_KEY).analyze(request).await.unwrap_err());
    Ok(())
}
